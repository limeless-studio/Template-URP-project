...
namespace Core.Attributes.Editor
{
    ...
    public class SnAttrInspector : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            bool anySnAttribute = _serializedProperties.Any(p => PropertyUtility.GetAttribute<ISnAttribute>(p) != null);
            if (!anySnAttribute)
            {
        ...
        protected void DrawSerializedProperties()
        {
            ...
            foreach (var property in GetNonGroupedProperties(_serializedProperties))
            {
                ...
                {
                    SnAttrEditorGUI.PropertyField_Layout(property, includeChildren: true);
                }
            }
            ...
            foreach (var group in GetGroupedProperties(_serializedProperties))
            {
                ...
                SnAttrEditorGUI.BeginBoxGroup_Layout(group.Key);
                foreach (var property in visibleProperties)
                {
                    SnAttrEditorGUI.PropertyField_Layout(property, includeChildren: true);
                }

                SnAttrEditorGUI.EndBoxGroup_Layout();
            }
            ...
            foreach (var group in GetFoldoutProperties(_serializedProperties))
            {
                ...
                if (_foldouts[group.Key].Value)
                {
                    foreach (var property in visibleProperties)
                    {
                        SnAttrEditorGUI.PropertyField_Layout(property, true);
                    }
                }
            }
        ...
        protected void DrawNonSerializedFields(bool drawHeader = false)
        {
            if (_nonSerializedFields.Any())
            {
                if (drawHeader)
                {
                    ...
                    SnAttrEditorGUI.HorizontalLine(
                ...
                {
                    SnAttrEditorGUI.NonSerializedField_Layout(serializedObject.targetObject, field);
                }
            }
        }
        ...
        protected void DrawNativeProperties(bool drawHeader = false)
        {
            if (_nativeProperties.Any())
            {
                ...
                {
                    SnAttrEditorGUI.NativeProperty_Layout(serializedObject.targetObject, property);
                }
            }
        }
        ...
        protected void DrawButtons(bool drawHeader = false)
        {
            if (_methods.Any())
            {
                ...
                {
                    SnAttrEditorGUI.Button(serializedObject.targetObject, method);
                }
            }
        }
